...
def run_eco_gallery():
    ...
    # Live camera feed with external camera (index 1)
    capture = st.button("📸 Take Photo")

    # Initialize video capture outside the button to persist state
    ...
    if ret:
        ...
        st.image(frame_rgb, channels="RGB", caption="Live Feed")

    # On capture
    if capture and ret:
        img_array = frame_rgb.copy()  # already in RGB
    ...
    # Optional: release camera when done (could also handle in teardown)
    # st.session_state.cap.release()  # Don't auto-release unless explicitly closing the page
